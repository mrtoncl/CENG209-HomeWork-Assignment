...
void initMap(Map *map) {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            map->rooms[i][j] = malloc(sizeof(Room));  // Allocate memory for the room
            strcpy(map->rooms[i][j]->description, descriptions[i][j]);  // Set room description
            map->rooms[i][j]->up = map->rooms[i][j]->down = NULL;
            map->rooms[i][j]->left = map->rooms[i][j]->right = NULL;
            map->rooms[i][j]->creature = NULL;  // No creature initially
            map->rooms[i][j]->item = NULL;      // No item initially
        }
    }

    // Connect rooms in all directions
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
    ...
    // Add creatures, items, and traps to specific rooms for demonstration
...
// Returns a room based on the coordinates
Room* getRoom(Map *map, int x, int y) {
    if (x >= 0 && x < 3 && y >= 0 && y < 3) {
        return map->rooms[x][y];  // Return room at coordinates (x, y)
    }
